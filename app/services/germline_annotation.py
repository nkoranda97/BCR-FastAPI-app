...
def parse_kabat_annotation(gene: str, species: str) -> Optional[dict]:
    ...
    print(
        f"[DEBUG] Searching for gene '{gene}' (base '{base_gene}') in {path} with pattern {pattern.pattern}"
    )
    found = False
    with open(path) as f:
        for line in f:
    ...
    if not found:
        print(
            f"[WARN] No Kabat annotation found for gene '{gene}' (base '{base_gene}') in {path}"
        )
    return None
...
def get_germline_sequence(gene: str, species: str) -> Optional[str]:
    ...
    print(
        f"[WARN] No germline FASTA record found for {gene} (base {base_gene}) in {path}"
    )
    return None
