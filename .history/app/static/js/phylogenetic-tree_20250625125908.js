...
class PhylogeneticTree {
  ...
  update(newick) {
    ...
    nodeUpdate.select('text')
      ...
      .text(d => {
        ...
        if (this.opts.showDistances) {
          // Calculate total distance from root to this leaf
          const totalDist = d.ancestors()
            .slice(1) // Skip self
            .reduce((sum, node) => sum + (node.data.length || 0), 0);
          text += totalDist > 0 ? ` (${totalDist.toFixed(3)})` : '';
        }
        return text;
      });
  ...
  #toggleNodeCollapse(d) {
    ...
    nodeUpdate.select('text')
      ...
      .style('text-anchor', 'start')
      .text(n_d => {
        let text = n_d.data.name ?? '';
        if (this.opts.showDistances) {
          const totalDist = n_d.ancestors()
            .slice(1)
            .reduce((sum, node) => sum + (node.data.length || 0), 0);
          text += totalDist > 0 ? ` (${totalDist.toFixed(3)})` : '';
        }
