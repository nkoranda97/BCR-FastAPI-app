...
class PhylogeneticTree {
  ...
  #refreshDims() {
    ...
    // Compute vertical available space (excludes margins)
    const availableVertical = containerHeight - this.opts.margin.top - this.opts.margin.bottom;
    // Compute spacing per leaf, reducing for large trees
    let vSep = this.opts.verticalNodeSeparation;
    if (this.root && this.leafCount > 1) vSep = Math.min(vSep, availableVertical / this.leafCount);
    // Determine desired and minimal heights, allowing overflow beyond container
    const desiredTreeHeight = this.root ? this.leafCount * vSep : 0;
    const minHeightForLeaves = this.root ? Math.max(vSep * Math.min(5, this.leafCount), this.opts.fontSize * 3 * Math.min(5, this.leafCount)) : 0;
    // Layout height purely based on tree content (ignore availableVertical)
    const layoutHeight = Math.max(desiredTreeHeight, minHeightForLeaves, this.opts.fontSize * 5);
